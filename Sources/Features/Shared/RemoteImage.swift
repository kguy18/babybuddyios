...
/// (same-origin — never leaked to a third-party host). The custom headers follow the same rule, so
/// an access gate in front of `/media/` lets the fetch through.
///
...
actor ImageLoader {
    ...
    func image(for url: URL, token: String?, headers: [CustomHeader] = []) async -> UIImage? {
        ...
        guard let (data, response) = try? await session.data(
            for: request, delegate: CustomHeaderRedirectGuard.make(for: headers)) else { return nil }
...
struct RemoteImage<Placeholder: View>: View {
    ...
    private func load() async {
        ...
        image = await ImageLoader.shared.image(for: resolved.url, token: resolved.token, headers: resolved.headers)
    }

    /// Resolve a serialized media URL into a fetchable URL plus the token and custom headers to send
    /// (if any). Handles absolute http/https (same-host `http`→`https` upgrade, same-host token and
    /// headers), `file://` (demo), and server-relative paths. Returns `nil` for empty/unusable input.
    static func resolve(_ urlString: String?, config: ServerConfig?)
        -> (url: URL, token: String?, headers: [CustomHeader])? {
        ...
        if var comps = URLComponents(string: raw), let scheme = comps.scheme?.lowercased() {
            ...
            return sameHost ? (url, config?.token, config?.headers ?? []) : (url, nil, [])
        }
        ...
        return (url, config?.token, config?.headers ?? [])
    }
}
